...
public class MotionProfileHelper {
	 ...
	 * keep one copy.
	 */
	private MotionProfileStatus profileStatus = new MotionProfileStatus();
	...
	/**
	 * Since the TalonFX.set() routine is mode specific, deduce what we want
	 ...
	 * to do this, just call falcon.processMotionProfileBuffer() in your teleop loop.
	...
	class PeriodicRunnable implements java.lang.Runnable {
	    public void run() {  falcon.processMotionProfileBuffer();    }
	}
	...
	public MotionProfileHelper(TalonFX talon, double[][] profile, int total, boolean invert) {
        falcon = talon;
        _total=total;
		_profile=profile;
		inverted=invert;
	...
	public void control() {
		...
		falcon.getMotionProfileStatus(profileStatus);
		...
		if (falcon.getControlMode() != ControlMode.MotionProfile) {
			/*
			...
			switch (_stateLeft) {
				...
				case 1: /*
					...
					if (profileStatus.btmBufferCnt > kMinPointsInTalon) {
				...
				case 2: /* check the status of the MP */
					 ...
					 */
					if (profileStatus.isUnderrun == false) {
						_loopTimeout = kNumLoopsTimeout;
					}
				...
				//TODO: see comment
					...
					if (profileStatus.activePointValid && profileStatus.isLast){//profileStatus.activePoint.isLastPoint) {
						/*
		...
		//instrumentation.process(profileStatus);
	}
	...
	private void startFilling(double[][] profile, int totalCnt) {
		...
		for (int i = 0; i < totalCnt; ++i) {
			...
			falcon.pushMotionProfileTrajectory(point);
		}
	}
